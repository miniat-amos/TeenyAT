...
constant_immediate(val) ::= IDENTIFIER(id). {
	/*
	 * Because a constant_immediate is only used to set a constant or
	 * as a particular value in a raw_line, identifiers here must be
	 * already defined constants.
	 */
	symbol *sym;
	if((sym = symbol_table_search(constants, id->token_str))) {
		val = sym->value;
		most_recent_const_immed_line_num = id->line_num;
	} else {
		STAGE_ERROR(id->line_num, "Unknown identifier \"%s\"", id->token_str);
	}
}
